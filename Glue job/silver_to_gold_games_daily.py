...
raw_games_df = spark.read.format("delta").load(SILVER_PATH)
feature_frame = build_feature_frame(raw_games_df).select(*historical_output_columns)
today_feature_rows = feature_frame.filter(F.col("GAME_DATE") == F.lit(process_date)).dropDuplicates(["GAME_ID", "TEAM_ID"])

historical_exists = DeltaTable.isDeltaTable(spark, HISTORICAL_FEATURE_PATH)

if historical_exists:
    if not today_feature_rows.rdd.isEmpty():
        historical_table = DeltaTable.forPath(spark, HISTORICAL_FEATURE_PATH)
        (
            historical_table.alias("target")
            .merge(
                today_feature_rows.alias("source"),
    ...
    current_history_df = spark.read.format("delta").load(HISTORICAL_FEATURE_PATH)
else:
    feature_frame.write.format("delta").mode("overwrite").partitionBy("SEASON").save(HISTORICAL_FEATURE_PATH)
    current_history_df = feature_frame


inference_source_df = latest_team_snapshot(current_history_df)
...
if inference_source_df is not None:
    ...
    print(f"Historical feature rows: {current_history_df.count()}")
    print(f"Inference snapshot rows: {inference_df.count()}")
else:
    print("No rows were available to build the inference snapshot.")

matchup_lookup_df = (
    current_history_df.select(
...
print(f"Matchup lookup rows: {matchup_lookup_df.count()}")
